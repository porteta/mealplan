 ...
 */
AmpersandFire._getKey = function(refOrSnapshot) {
 ...
 */
AmpersandFire._determineAutoSync = function(model, options) {
 ...
 */
AmpersandFire.sync = function(method, model, options) {
    var modelJSON = model.toJSON();

    if (method === 'read') {

        AmpersandFire._readOnce(model.firebase, function onComplete(snap) {
            var resp = snap.val();
            if (options.success) {
        ...
        AmpersandFire._setWithCheck(model.firebase, modelJSON, options);

    } else if (method === 'update') {

        AmpersandFire._updateWithCheck(model.firebase, modelJSON, options);

    } else if (method === 'delete') {

        AmpersandFire._setWithCheck(model.firebase, null, options);
 ...
 */
AmpersandFire._readOnce = function(ref, onComplete) {
 ...
 */
AmpersandFire._setToFirebase = function(ref, item, onComplete) {
 ...
 */
AmpersandFire._updateToFirebase = function(ref, item, onComplete) {
 ...
 * from Firebase.
 */
AmpersandFire._onCompleteCheck = function(err, item, options) {
    if (!options) {
        return;
    }
 ...
 */
AmpersandFire._setWithCheck = function(ref, item, options) {
    AmpersandFire._setToFirebase(ref, item, function(err) {
        AmpersandFire._onCompleteCheck(err, item, options);
    });
};
 ...
 * error events from the server.
 */
AmpersandFire._updateWithCheck = function(ref, item, options) {
    AmpersandFire._updateToFirebase(ref, item, function(err) {
        AmpersandFire._onCompleteCheck(err, item, options);
    });
};
 ...
 */
AmpersandFire._throwError = function(message) {
 ...
 */
AmpersandFire._determineRef = function(objOrString) {
    switch (typeof(objOrString)) {
        ...
        case 'object':
            return objOrString;
        default:
            AmpersandFire._throwError('Invalid type passed to url property');
    }
};
 ...
 */
AmpersandFire._checkId = function(snap) {
    var model = snap.val();

    // if the model is a primitive throw an error
    if (AmpersandFire._isPrimitive(model)) {
        AmpersandFire._throwError('InvalidIdException: Models must have an Id. Note: You may ' +
    ...
    model.id = AmpersandFire._getKey(snap);
 ...
 */
AmpersandFire._isPrimitive = function(value) {
 ...
 */
AmpersandFire._promiseEvent = function(params) {
 ...
 * This model is never directly used. The AmpersandFire.Model will
    ...
    SyncModel.protoype = {
        fetch: function(options) {
            AmpersandFire._promiseEvent({
                ...
                complete: function() {
                    AmpersandFire._onCompleteCheck(this._initialSync.err, this, options);
                }
            });
        }
    };
...
AmpersandFire.Model = AmpersandModel.extend({
    ...
    constructor: function(model, options) {
        ...
        this.autoSync = AmpersandFire._determineAutoSync(this, options);

        switch (typeof this.url) {
            case 'string':
                this.firebase = AmpersandFire._determineRef(this.url);
                break;
            case 'function':
                this.firebase = AmpersandFire._determineRef(this.url());
                break;
            case 'object':
                ...
                break;
            default:
                AmpersandFire._throwError('url parameter required');
        }
    ...
    _setId: function(snap) {
        ...
        if (this.isNew()) {
            this.set('id', AmpersandFire._getKey(snap), {
                silent: true
            });
        }
    },
    ...
    _unsetAttributes: function(snap) {
        var newModel = AmpersandFire._checkId(snap);
...
var OnceCollection = (function() {
    ...
    OnceCollection.protoype = {
         ...
         * AmpersandFire.sync with the correct method.
         */
        create: function(model, options) {
            model.id = AmpersandFire._getKey(this.firebase.push());
            options = _.extend({
                autoSync: false
            }, options);
            return AmpersandCollection.prototype.create.call(this, model, options);
        },
        /**
        ...
        add: function(model, options) {
            ...
            return AmpersandCollection.prototype.add.call(this, model, options);
        },
        /**
         * Proxy to AmpersandFire.sync
         */
        sync: function(method, model, options) {
            AmpersandFire.sync(method, model, options);
        },
         ...
         * AmpersandCollection.fetch method by mapping the returned object from
    ...
    SyncCollection.protoype = {
        ...
        remove: function(models, options) {
            ...
            for (var i = 0; i < parsed.length; i++) {
                ...
                AmpersandFire._setWithCheck(childRef, null, options);
            }
        ...
        _parseModels: function(models, options) {
            ...
            for (var i = 0; i < models.length; i++) {
                ...
                model = AmpersandCollection.prototype._prepareModel.call(
                    this, model, options
                );
        ...
        _childAdded: function(snap) {
            var model = AmpersandFire._checkId(snap);
        ...
        _childRemoved: function(snap) {
            ...
            if (this._suppressEvent === true) {
                this._suppressEvent = false;
                AmpersandCollection.prototype.remove.call(
                    this, [model], {
                        silent: true
                    }
                );
                ...
                this.trigger('sync', this);
                AmpersandCollection.prototype.remove.call(this, [model]);
            }
        },
        ...
        _removeModel: function(model, collection, options) {
            ...
            options.success =
                _.isFunction(options.success) ? options.success : function() {};
            var childRef = this.firebase.child(model.id);
            AmpersandFire._setWithCheck(childRef, null, _.bind(options.success, model));
        },
...
AmpersandFire.Collection = AmpersandCollection.extend({
    ...
    constructor: function(model, options) {
        ...
        this.model = function(attrs, opts) {
            ...
            newItem.sync = AmpersandFire.sync;
            newItem.on('change', function(model) {
                var updated = AmpersandFire.Model.prototype._updateModel(model);
                model.set(updated, {
                    silent: true
                });
            });
